...
function encabezado () {
	...
	obtenerJSON(urlAmiiboGame).then(json => { 
	
		enlaceDiv[0].innerText = json.amiibo[0].name;
		enlaceDiv[1].innerText = json.amiibo[5].name; 
		enlaceDiv[2].innerText = json.amiibo[7].name; 
		enlaceDiv[3].innerText = json.amiibo[74].name;  
...
/*function filtrarJuego(){
	...
	let array = new Array(json.amiibo.length); 
...
function filtrarJuego(){
	...
	obtenerJSON(urlAmiiboGameNombre).then(json => {
		...
		for (var i = 0; i < json.amiibo.length-1; i++) {

			if((json.amiibo[i].name != "Animal Crossing" )&& (json.amiibo[i].name != "Pokemon" ) && (json.amiibo[i].name != "Super Mario" )){
				var valor = json.amiibo[i+1].name;
				var opcion = document.createElement('option');
				opcion.appendChild( document.createTextNode(valor) );
				opcion.value = valor;
				categorias.appendChild(opcion); 
